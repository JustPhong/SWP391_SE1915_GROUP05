...
export const WelcomePage: React.FC = () => {
  ...
  useEffect(() => {
    if (user && activeTab === 'vehicles' && historyTab) {
      setHistoryLoading(true);
      getHistory().then(setHistory).finally(() => setHistoryLoading(false));
    }
  }, [user, activeTab, historyTab]);
  ...
  // ═══════════════════════════════════════════════════════════
  //  RENDER — LOGGED OUT: always show home tab
  // ═══════════════════════════════════════════════════════════
  if (!user) {
    return (
  ...
  // ═══════════════════════════════════════════════════════════
  //  RENDER — LOGGED IN: show tab switcher
  // ═══════════════════════════════════════════════════════════
  return (
    <div className={styles.page}>
      {/* ── 1. NAV ─────────────────────────────────────── */}
      <nav className={styles.nav}>
        <div className={styles.navInner}>
          ...
          <div className={styles.navRight}>
            ...
            <button className={styles.btnGhost} onClick={() => { logout(); setActiveTab('home'); navigate('/'); }}>
              Đăng xuất
            </button>
          </div>
        </div>
        ...
        <div className={styles.vcbPage}>
          <div className={styles.vcbInner}>
            ...
            <div className={styles.vcbHeader}>
              ...
              <button className={styles.btnPrimary} onClick={() => setShowAddVehicle(true)}>
                + Thêm xe
              </button>
            </div>
              ...
              <>
                ...
                <div className={styles.vcbStatsRow}>
                  ...
                  <div className={styles.vcbStatCard}>
                    ...
                    <div>
                      <div className={styles.vcbStatValue}>
                        {pkg ? daysLeft(pkg.expiryDate) + ' ngày' : 'Chưa có'}
                      </div>
                      <div className={styles.vcbStatLabel}>
                        {pkg ? pkg.planName : 'Gói tháng'}
                      </div>
                    </div>
                  </div>
                </div>
                    ...
                    <div className={styles.vcbVehicleList}>
                      {vehicles.map(v => (
                        <div key={v.id} className={styles.vcbVehicleCard}>
                          ...
                          <button
                            className={styles.vcbVehicleRemove}
                ...
                <div className={styles.vcbSection}>
                  <button
                    className={styles.vcbSectionToggle}
                    onClick={() => setHistoryTab(h => !h)}
                  >
                    <h2 className={styles.vcbSectionTitle} style={{ margin: 0 }}>
                      Lịch sử gửi xe
                    </h2>
                    <svg
                      width="16" height="16" viewBox="0 0 24 24" fill="none"
                      stroke="currentColor" strokeWidth="2"
                      style={{ transform: historyTab ? 'rotate(180deg)' : 'rotate(0deg)', transition: 'transform 0.2s' }}
                    >
                  ...
                  {historyTab && (
                    historyLoading ? (
                      <div className={styles.vcbLoading} style={{ padding: '2rem' }}>
                        <div className={styles.spinner} />
                      </div>
                    ) : history.length === 0 ? (
                      <div className={styles.vcbEmpty} style={{ padding: '2rem' }}>
                        <p>Chưa có lịch sử gửi xe.</p>
                      </div>
                    ) : (
                        ...
                        {history.map(h => (
                          <div key={h.id} className={styles.vcbHistoryItem}>
                            <div className={styles.vcbHistoryLeft}>
                              ...
                              <div className={styles.vcbHistoryMeta}>
                                Chỗ {h.slotCode} · {formatDate(h.date)}
                              </div>
                            </div>
            ...
            <form onSubmit={handleAddVehicle} className={styles.modalForm}>
              ...
              <div className={styles.formGroup}>
                ...
                <div className={styles.typeSelector}>
                  <button
                    type="button"
                    className={`${styles.typeOption} ${newType === 'CAR' ? styles.typeOptionActive : ''}`}
                  ...
                  <button
                    type="button"
                    className={`${styles.typeOption} ${newType === 'MOTORBIKE' ? styles.typeOptionActive : ''}`}
                    onClick={() => setNewType('MOTORBIKE')}
                  >
  ...
  return (
    <div className={`${styles.pricingCard} ${featured ? styles.pricingCardFeatured : ''}`}>
      ...
      <div className={styles.pricingHeader}>
        <span className={styles.pricingTier}>{title}</span>
      </div>
      ...
      <button className={`${styles.pricingBtn} ${featured ? styles.pricingBtnFeatured : ''}`} onClick={onClick}>
        Chọn gói
      </button>
    </div>
  );
