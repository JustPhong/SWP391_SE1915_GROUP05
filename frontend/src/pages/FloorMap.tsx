...
function IconBike({ size = 16, color = C.white }: { size?: number; color?: string }) {
            ...
            return (
              ...
              >
                {floor.vehicleType === 'CAR'
                  ? <IconCar size={13} color={active ? C.white : C.navy} />
                  : <IconBike size={13} color={active ? C.white : C.navy} />}
                {floor.name}
                {isT3 && (
                  <span style={{
                    fontSize: '0.6rem',
